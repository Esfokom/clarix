...
class _ExtractPagesDialogState extends ConsumerState<ExtractPagesDialog> {
    ...
    try {
      ...
      if (mounted && _source == source) {
        setState(() {
          ...
          _operationError =
              'Could not load ${path.basename(source)}: ${utilityMessageFor(error)}';
        });
      }
    } finally {
      if (mounted && _source == source) {
  ...
  Future<void> _extract() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => _operationError = utilityMessageFor(error));
      }
    } finally {
      if (mounted) {
        setState(() => _busy = false);
...
Future<String?> pickExportPdf() async {
  final FilePickerResult? result = await FilePicker.pickFiles(
...
Future<String?> pickExportDestination(UtilityFormat format, String sourcePath) {
...
Future<List<String>> pickConversionSources() async {
  final FilePickerResult? result = await FilePicker.pickFiles(
...
Future<String?> pickConversionOutputDirectory() => FilePicker.getDirectoryPath(
...
String utilityMessageFor(Object error) {
